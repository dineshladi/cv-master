...
print_section <- function(position_data, section_id){
  position_data %>% 
    ...
    mutate(
      ...
      project_points = ifelse(is.na(project), description_bullets, paste(glue("<b>{project}</b>"), description_bullets,sep = "\n\n"))
    ) %>% 
    group_by(section, in_resume, title, institution, loc, timeline, end) %>% 
    summarise(company_points = paste(project_points, collapse = "\n\n")) %>% 
    ungroup() %>% 
    arrange(desc(end)) %>% 
    strip_links_from_cols(c('title', 'company_points')) %>% 
